...
func (s *LoginServer) RegisterHandlers(rootMux *http.ServeMux) error {
	...
	mux.Handle("GET /auth/token/{$}", s.middlewareNoAuthCheck(s.HandleGetToken))
	mux.Handle("GET /auth/token/validate/{$}", s.middlewareNoAuthCheck(s.HandleValidateToken))
...
func (s *LoginServer) HandleOAuthCallback(w http.ResponseWriter, r *http.Request) error {
	...
	_, err = s.createOrInsertUser(r.Context(), claims.Sub)
	if err != nil {
		log.Println(err)
		http.Redirect(w, r, "/login/callback?status=failed", http.StatusTemporaryRedirect)
		return err
	...
	if oldToken, ok := session.Values[auth.OAUTH_TOKEN_KEY].(*oauth2.Token); ok {
		...
		s.authService.RevokeSessionToken(oldToken)
	}
...
func (s *LoginServer) HandleValidateToken(w http.ResponseWriter, r *http.Request) error {
	...
	_, err := s.authService.ValidateJWTToken(tokenString)
	if err != nil {
		return err
	} else {
		return nil
...
func (s *LoginServer) HandleGetToken(w http.ResponseWriter, r *http.Request) error {
	...
	tokenString, err := s.authService.CreateJWTToken(
		authInfo.User.UserId,
		authInfo.User.TwitchUserId,
...
func (s *LoginServer) createOrInsertUser(ctx context.Context, twitchUserIdStr string) (uint, error) {
	// Insert into Users table
	userInfo, err := s.authService.GetUserFromTwitchId(twitchUserIdStr)
	if err != nil {
		return 0, err
	}
	userDb, err := s.usersRepo.GetOrInsertUser(ctx, *userInfo)
	if err != nil {
		return 0, err
	}
